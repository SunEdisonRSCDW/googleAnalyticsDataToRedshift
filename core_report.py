...
__author__ = 'rbagayatkar@sunedison.com (Raj Bagayatkar)'
...
def rows_to_rs(results):
  ...
  if results.get('rows', []):
    cursor = conn.cursor();
    for output in results['rows']: 
      query = "INSERT INTO ga_test (browser,new_users,percent_new_sessions,sessions,bounce_rate,avg_session_duration,pages_per_session) values ('%s'"%output[0] + ",%d"%int(float(output[1])) + ",'%.2f'"%float(output[2]) + ",%d"%int(float(output[3])) + ",'%.2f'"%float(output[4]) + ",'%.2f'"%float(output[5]) + ",'%.2f'"%float(output[6]) + ")";
      try: 
        cursor.execute(query);
      except:
        traceback.print_exc(file=sys.stdout);
